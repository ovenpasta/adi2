...
gprbuild -P tests/tests.gpr -XTEST_KIND=layout_flex_grid_test
...
gprbuild -P tests/tests.gpr -XTEST_KIND=svg_test
gprbuild -P tests/tests.gpr -XTEST_KIND=svg_perf_test
...
gprbuild -P examples/examples.gpr -XEXAMPLE_KIND=material_demo
...
./tests/bin/layout_flex_grid_test
...
- Supports comma-separated selector groups (`li, ul, p { ... }`) by applying the same declarations to each selector
...
- In `Static_Mode`, repeated entries for the same selector are merged in insertion order (later declarations override earlier ones)
...
- Comma-separated selector groups (for example `li, ul, p { ... }`) are expanded per selector in generated output
- Generated packages include selector-type distinction in constants:
  - `*_Class_Part_Styles`
  - `*_Id_Part_Styles`
  - `*_Tag_Part_Styles`
  (plus per-style `*_Widget` constants)
- Repeated blocks for the same selector/state are merged with CSS ordering semantics (later declarations win)
...
- Comma-separated selector groups in one rule block are supported for class/id/tag selectors
...
Mode behavior:
- `Dynamic_Mode`: load CSS from disk (`Add_Dynamic_File`) and optionally auto-reload with `Tick`
- `Static_Mode`: apply compiled style entries (`Set_Static_Entries`) generated by `css_to_ada.py`
- `Static_Mode` merges repeated static entries for the same selector in declaration/insertion order
...
- `docs/build.md`
- `docs/css_styling.md`
- `docs/xml_ui_system.md`
- `docs/coding_conventions.md`
- `docs/gprbuild_without_alire.md`
- `docs/html_view_spec.md`
