...
| `perf_stats()` | Frame timing, FPS, layout counts, texture residency, and `style_stores` occupancy (`rule_sets`, `styles`, `resolved`, `text`, `gradients`, `widget_properties`, `fonts`, each `count`/`bytes`, plus `resolved_cap`, `resolved_generation`, `font_idle_bytes`, `font_budget`, `font_evictions`) |
