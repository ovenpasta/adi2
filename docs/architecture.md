...
- Overlay state cleanup: `Remove_Overlay` and `Clear_Overlays` release focus, hover and pressed when they point into an overlay being removed, and clear the `State_Hovered`/`State_Pressed` those widgets hold, so an overlay shown again comes back in the state it would have if it had just been built. Hover stops at the subtree's parent: the pointer has not moved, so that ancestor takes over as the hovered widget. `Clear_Widget_Refs_In_Subtree` does the same for one widget's subtree and is what `Destroy` calls before detaching
...
- **Mouse coordinate space**: `On_Mouse_Down`, `On_Mouse_Move` and `On_Mouse_Up` receive points in **stored layout space** — the same space `Get_Geometry` reports, not zero-based widget-local coordinates. `Map_Window_Point_To_Widget` converts a window point by adding the scroll offset of every ancestor of the target (the target's own offset moves its children, not itself), so the delivered point falls inside the widget's stored rectangle however deeply it is scrolled. X passes through unchanged because no widget carries a horizontal scroll offset — `overflow-x` still clips its axis in `Render`, still feeds `Scrollable_X` in `Get_Preferred_Size`, and still drives `Axis_Minimum` in `Effective_Min_Size`; it just never produces an offset to undo. Context-menu coordinates (`Bubble_Context_Menu`) stay in window space: they position an overlay, not content inside the widget. `tests/src/window_resize_safety_test.adb` pins the contract through two nested scroll containers.
...
- A tick descends into a widget while it or something under it wants one. `Request_Tick` raises that along the whole parent chain, so a widget wanting a tick is reachable from every root above it, and a widget's own tick lowers it having asked its transitions, its scrolling and its subtree what is left. `Mark_Dirty` and `Mark_Render_Dirty` raise it on the walk they already make, and `Apply_Styles_To_Items` raises it where a transition starts. An overriding `On_Tick` with work of its own calls `Request_Tick` where that work runs, as the three animation widgets do; `List_Box` and `Text_Editor` override it to scroll and nothing else, which the scroll answer already covers. A type registered through `Adi.Widget.Extension` is ticked always, the library having no way to ask a type it never saw
...
`Resolved_Flex_Base` answers what main-axis size an item is laid out from before it grows or shrinks: a definite `flex-basis` exactly — including `0`, a demand for nothing — a percentage main size resolved against the container's content box, and otherwise the item's content size at its assigned width. Aggregation and the flex algorithm both resolve the base through it, so a container cannot reserve room on one rule while layout places on another. `Container_Main` is always the container's own content box and never the item's geometry, so no pass can narrow the base the pass before it produced; a container still being sized passes zero, and the percentage yields to the content there as CSS does.
