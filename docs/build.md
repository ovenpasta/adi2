...
(cd demos && alr build -- -j0)  # regenerate and build all demos
```
...
`tools/build_demos.sh` runs the generators for you; when driving
...
bash tools/generate_demo_translations.sh
```
...
# Demos (Alire helper, or standalone DEMO_KIND scenario variable)
tools/build_demos.sh label_example
...
### Valid DEMO_KIND values
The Alire crate builds the `Main` list in `demos/demos.gpr`. The standalone
configure build uses the `Demo_Kind` enumeration generated by `tools/configure.sh`.

## Running

```bash
./tests/bin/<test_name>       # e.g. ./tests/bin/styles
./demos/bin/<example_name> # e.g. ./demos/bin/label_example
...
- `projects/{adi_build.gpr, tests_build.gpr, demos_build.gpr}`, and `plutosvg_build.gpr` and `rlottie_build.gpr` for the vendored C libraries `adi_build.gpr` withs
