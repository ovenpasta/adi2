...
| `grid-template-rows` | track list read for its length, `repeat(N, size)`, or `none` | `grid-template-rows: repeat(2, 1fr);` |
...
- `grid-template-columns: none` is the property's initial value and names both: a count of zero and a track list of no tracks. Both are named values rather than absences, so a rule declaring `none` takes the tracks off a less specific rule.
- A value naming a token outside the table above is invalid, and the whole declaration is dropped: `grid-template-columns: red blue` leaves the property to the cascade rather than becoming two columns. `grid-template-rows` reads the same track grammar and takes the list's length as its row count.
- A track list past the 16 the array holds is read and its count kept, with the sizes given up; the count then travels alone and the tracks are left to a less specific rule.

**Ada static form** (generated by `css_to_ada.py` and usable directly in Ada):

```ada
Grid_Column_Tracks => Set ((Count => 4,
...
`Set` of a list of no tracks is the cleared track key, which is what `none`
comes to.
...
`Style_Rules`, 1,080 bytes on x86-64. A rule naming three properties, the
...
`Grid_Column_Tracks` is the one value holding no `CSS_Property` literal of
its own. It travels with `grid-template-columns` as that property's second
part, and `Grid_Columns` carries an overload for each: one taking the
track count, one taking the list.

The generator emits chains, so a generated sheet names `Style_Rules`
nowhere.
...
rule was written as materialises 1,080. A side longhand takes a step per
