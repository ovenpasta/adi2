...
- `fr` columns receive a proportional share of the space left after all `auto` and `px` columns are sized. An `fr` column's minimum is 0 — it never forces the grid wider than the container (unlike CSS `minmax(min-content, 1fr)`).
