...
- **Class CSS replaces widget intrinsic defaults** — Some widgets (`Html_View`, `Text_Editor`, `List_Box`) set up their own internal `Part_Style_Array` via `Set_Part_Styles (Default_Internal_Part_Styles)` during `Create` — e.g. `Html_View` defaults to `overflow-x: auto; overflow-y: auto`. When `Adi.CSS_Source.Apply_To_Widget` later applies a class binding, it calls `Set_Part_Styles` again, which **replaces** the part-style array rather than merging on top of the intrinsics. Any default the widget set up is lost. The visible symptom: a class that doesn't restate `overflow-y: auto` on an `Html_View` leaves the resolved overflow at the spec default `visible`, and the widget stops clipping + scrolling. **Workaround**: restate the intrinsic property explicitly in your CSS class. To fix properly: add an `Intrinsic_Part_Styles (W) return Part_Style_Array` primitive on `Widget'Class` (default empty; overrides in `Html_View` / `Text_Editor` / `List_Box`) and have `Apply_To_Widget` merge intrinsic + user styles before calling `Set_Part_Styles`. Then the widget's `overflow-y: auto` survives class binding and users only restate it to *override*.
