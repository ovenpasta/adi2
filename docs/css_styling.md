...
- Comma-separated selector groups are supported (`li, ul, p { ... }`) and expanded per selector
...
Comma-separated selector groups are supported for class/id/tag selectors.
...
- In `Static_Mode`, repeated entries for the same selector are merged in insertion order (later entries win)
