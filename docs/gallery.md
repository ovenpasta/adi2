# Example gallery

Each example program in [`examples/`](../examples/), captured as a screenshot. Build any of them with `tools/build_examples.sh <name>` and run from `examples/bin/<name>`.

![animated_image_example](../examples/screenshots/animated_image_example.png)
*`animated_image_example`*

![assets_example](../examples/screenshots/assets_example.png)
*`assets_example`*

![button_example](../examples/screenshots/button_example.png)
*`button_example`*

![combo_box_example](../examples/screenshots/combo_box_example.png)
*`combo_box_example`*

![demo_flex](../examples/screenshots/demo_flex.png)
![demo_flex](../examples/screenshots/demo_flex_2.png)
*`demo_flex`*

![dialog_example](../examples/screenshots/dialog_example.png)
![dialog_example](../examples/screenshots/dialog_example_2.png)
*`dialog_example`*

![font_example](../examples/screenshots/font_example.png)
![font_example](../examples/screenshots/font_example_2.png)
*`font_example`*

![gradient_example](../examples/screenshots/gradient_example.png)
*`gradient_example`*

![grid_example](../examples/screenshots/grid_example.png)
![grid_example](../examples/screenshots/grid_example_2.png)
*`grid_example`*

![hello_example](../examples/screenshots/hello_example.png)
*`hello_example`*

![html_view_example](../examples/screenshots/html_view_example.png)
![html_view_example](../examples/screenshots/html_view_example_2.png)
*`html_view_example`*

![image_example](../examples/screenshots/image_example.png)
*`image_example`*

![label_example](../examples/screenshots/label_example.png)
*`label_example`*

![list_box_example](../examples/screenshots/list_box_example.png)
*`list_box_example`*

![material_demo](../examples/screenshots/material_demo.png)
![material_demo](../examples/screenshots/material_demo_2.png)
![material_demo](../examples/screenshots/material_demo_3.png)
![material_demo](../examples/screenshots/material_demo_4.png)
*`material_demo`*

![overflow_example](../examples/screenshots/overflow_example.png)
*`overflow_example`*

![rlottie_example](../examples/screenshots/rlottie_example.png)
*`rlottie_example`*

![runtime_css_example](../examples/screenshots/runtime_css_example.png)
*`runtime_css_example`*

![slider_example](../examples/screenshots/slider_example.png)
*`slider_example`*

![stack_example](../examples/screenshots/stack_example.png)
*`stack_example`*

![svg_example](../examples/screenshots/svg_example.png)
*`svg_example`*

![text_editor_example](../examples/screenshots/text_editor_example.png)
*`text_editor_example`*

![text_input_example](../examples/screenshots/text_input_example.png)
*`text_input_example`*

![transition_example](../examples/screenshots/transition_example.png)
*`transition_example`*

![value_input_example](../examples/screenshots/value_input_example.png)
*`value_input_example`*
