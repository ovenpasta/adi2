...
How to inspect and drive a running Adi application over MCP: screenshots, widget introspection, search, interaction and CSS inspection.
...
| `widget_tree()` | Full widget hierarchy as JSON: `tree` and `overlays`, each node carrying `type`, `id`, `path`, `text`, `x`/`y`/`w`/`h`, `states`, `visible`/`clickable`/`focusable`, `child_count`, `items_count` and `children` |
| `widget_info(id, path)` | Detailed info for one widget (by ID or path) |
| `perf_stats()` | Frame timing and FPS, and the per-frame counters: `style_hits`, `style_memo_hits` and `style_computes` partition `style_resolves` between the per-widget cache, the global memo and the cascade; `layout_calls`, `layout_skips`, `pref_calls`, `pref_hits`; `tick_visits`, the widgets the animation tick reached; `selector_memo_hits`/`selector_memo_misses` for the memo over the styles a (tag, classes, id) triple folds to in `Adi.CSS_Source`. A frame's counters run from the end of the frame before it: the events dispatched between the two draws, the animation tick, and this frame's own update, layout and drawing. A `texture_cache` object carries budget, total/idle/peak bytes, and per-producer (`shadow`, `raster`, `svg`, `view`) active/idle/retired residency, hits, misses, stores, `pressure_evictions`, `headroom_evictions`, `crowded_evictions` (borrowed entries past the count that bounds them), `released` (dropped with their group) and `build_us`. The budget bounds idle residency, so read `idle_bytes` against it |
| `perf_stats()` — `style_stores` | The same response carries a `style_stores` object: `rule_sets`, `styles`, `resolved`, `text`, `gradients`, `widget_properties` and `fonts`, each with a `count` and a `bytes`, plus `resolved_cap`, `resolved_generation`, `font_idle_bytes`, `font_budget` and `font_evictions`. Two stores evict. Read the resolved store's `count` against `resolved_cap` and watch `resolved_generation` for a clear; read `font_idle_bytes` against `font_budget`, and `font_evictions` rising across a steady scene says the budget sits below the working set — an open face holds an operating-system handle as well as its bytes. Every other store holds what it has for the life of the process, so a `count` that keeps climbing across a steady scene names the store something is feeding: a selector built per list row, a `background-image` URL or a selector name synthesised per row, a style interned from content rather than from a sheet. `widget_properties` is the exception: its bytes are a fixed registry, and only its count moves |
| `perf_stats()` — `style_chains` | The composer's fixed pool of chain buffers (`docs/css_styling.md`, Composing a Style in Ada): `open` now against `capacity`, `slot_capacity` for the properties one chain holds, and two life-of-process counters. `reclaimed` rises when a chain opening on a full pool had to take back the buffer held longest, which says a chain somewhere ended in neither `.Build` nor `.Discard` — an exception between `Style_Of` and `.Build` does it. `dropped_slots` rises when a chain named more than `slot_capacity` properties, and those properties are not in the style it built. Both stay at zero for a program whose chains finish, so either climbing is the finding |
