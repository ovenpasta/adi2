...
Metal has no overload: SDL adopts a `CVPixelBufferRef` there rather than
a texture, which does not fit this shape.

### Pixels from the CPU

`Set_Pixels` copies a buffer into a texture the view owns and reuses. It
works on every backend, since the view creates the texture through the
renderer it draws with.
...
A blinking caret is a few hundred pixels; the 4K surface holding it is
thirty megabytes to upload.

`Pitch` is the byte distance between rows of `Data`. The last row is
read to its width and no further, so `Pitch * (Rows - 1) + Columns * 4`
bytes is enough.
...
Letting go is also how an application says a handle is dead. A view
keeps the texture it wrapped a handle in, keyed by everything it was
told about it, so a producer rotating a pool of shared textures pays no
create and destroy per frame. A producer that frees a texture and is
handed **the same name or the same address back** for a new one — which
both GL and the D3D runtime do — would otherwise be given the wrapper
for what used to be there, and SDL would draw through a destroyed
object. Call `Clear_Texture` before you free.
...
be: usually that the texture's API differs from the one SDL built the
renderer with, otherwise that an upload was refused. The view draws
nothing in either case, so this is the only account of it.

A description SDL refuses is recorded once; the view retries nothing.
