...
package body Adi.Resolved_Styles is
   ...
   --  no indeterminate byte is ever touched.

   function Mix (H, V : Hash_Type) return Hash_Type is
     (H * 16#0100_0193# xor V);

   --  A float in a fixed-point key. The bound keeps the scaled value
   --  inside a 32-bit Integer, so a build whose Integer is 32 bits
   --  answers as this one does; a magnitude past it, and a NaN, fold to
   --  one key each and cost a bucket probe.
   function Num (F : Float) return Hash_Type is
     (if F /= F then 0
      elsif abs F >= 1.0E6 then 999_983
      else Hash_Type (Integer (Float'Truncation (F * 256.0)) mod 2 ** 24));

   function Add (H : Hash_Type; L : Length_Value) return Hash_Type is
     (Mix (Mix (H, Num (L.Amount)), CSS_Unit'Pos (L.Unit)));

   function Add (H : Hash_Type; C : Color_Value) return Hash_Type is
     (case C.Kind is
         when Named => Mix (Mix (H, 1), Named_Color'Pos (C.Name)),
         when RGB   => Mix (Mix (Mix (Mix (H, 2), Hash_Type (C.R)),
                                 Hash_Type (C.G)), Hash_Type (C.B)),
         when RGBA  => Mix (Mix (Mix (Mix (Mix (H, 3), Hash_Type (C.RA)),
                                      Hash_Type (C.GA)), Hash_Type (C.BA)),
                            Num (C.Alpha)));

   function Add (H : Hash_Type; S : Size_Value) return Hash_Type is
     (case S.Kind is
         when Fixed => Add (Mix (H, Size_Kind'Pos (S.Kind)), S.Size),
         when others => Mix (H, Size_Kind'Pos (S.Kind)));

   function Add (H : Hash_Type; I : Inset_Value) return Hash_Type is
     (case I.Kind is
         when Fixed => Add (Mix (H, 7), I.Length),
         when Auto  => Mix (H, 8));

   function Add (H : Hash_Type; M : Margin_Value) return Hash_Type is
     (case M.Kind is
         when Fixed => Add (Mix (H, 9), M.Length),
         when Auto  => Mix (H, 10));

   function Add (H : Hash_Type; B : CSS_Box_Value) return Hash_Type is
     (case B.Kind is
         when Gap_Uniform => Add (Mix (H, 11), B.All_Sides),
         when Axis        => Add (Add (Mix (H, 12), B.Vertical), B.Horizontal),
         when Per_Side    =>
           Add (Add (Add (Add (Mix (H, 13), B.Sides (Top)), B.Sides (Right)),
                     B.Sides (Bottom)), B.Sides (Left)));

   function Add (H : Hash_Type; W : Border_Width_Value) return Hash_Type is
     (case W.Kind is
         when Gap_Uniform => Add (Mix (H, 14), W.All_Edges),
         when Per_Edge    =>
           Add (Add (Add (Add (Mix (H, 15), W.Edges (Top)), W.Edges (Right)),
                     W.Edges (Bottom)), W.Edges (Left)));

   function Add (H : Hash_Type; C : Border_Color_Value) return Hash_Type is
     (case C.Kind is
         when Gap_Uniform => Add (Mix (H, 16), C.All_Edges),
         when Per_Edge    =>
           Add (Add (Add (Add (Mix (H, 17), C.Edges (Top)), C.Edges (Right)),
                     C.Edges (Bottom)), C.Edges (Left)));

   function Add (H : Hash_Type; R : Border_Radius_Value) return Hash_Type is
     (case R.Kind is
   ...
   function Add (H : Hash_Type; S : Border_Style_Value) return Hash_Type is
     (case S.Kind is
         when Gap_Uniform =>
   ...
   function Add (H : Hash_Type; G : Gap_Value) return Hash_Type is
     (case G.Kind is
         when Gap_Uniform  => Add (Mix (H, 22), G.All_Gap),
         when Gap_Separate =>
           Mix (Mix (Add (Add (Mix (H, 23), G.Row_Gap), G.Column_Gap),
                     Boolean'Pos (G.Has_Row)), Boolean'Pos (G.Has_Column)));

   function Add (H : Hash_Type; L : Line_Height_Value) return Hash_Type is
     (case L.Kind is
         when LH_Normal => Mix (H, 24),
         when LH_Number => Mix (Mix (H, 25), Num (L.Multiplier)),
         when LH_Length => Add (Mix (H, 26), L.Height));

   function Add (H : Hash_Type; F : Flex_Basis_Value) return Hash_Type is
     (case F.Kind is
         when Fixed  => Add (Mix (H, 27), F.Size),
         when others => Mix (Mix (H, 28), Flex_Basis_Kind'Pos (F.Kind)));

   function Add (H : Hash_Type; S : Box_Shadow_Value) return Hash_Type is
     (Add (Add (Add (Add (Add (Mix (H, 29), S.Offset_X), S.Offset_Y),
                     S.Blur_Radius), S.Spread_Radius), S.Color));

   function Add (H : Hash_Type; P : Object_Position_Value) return Hash_Type is
     (case P.Kind is
         when Keyword_Pos =>
           Mix (Mix (Mix (H, 30), Object_Position_Keyword'Pos (P.H_Keyword)),
                Object_Position_Keyword'Pos (P.V_Keyword)),
         when Length_Pos  => Add (Add (Mix (H, 31), P.X_Offset), P.Y_Offset));

   function Add (H : Hash_Type; T : Transition_Spec) return Hash_Type is
      Result : Hash_Type := Mix (Mix (H, Num (T.Duration)),
