...
package body Adi.Widget is
   ...
   type Resolved_Cache_Key is record
      Part_Handle        : Widget_Style := Empty_Widget_Style;
      Main_Handle        : Widget_Style := Empty_Widget_Style;
   ...
   function Hash_Resolved_Cache_Key
     ...
     (Resolved_Cache_Hash
        (Index (K.Part_Handle), Index (K.Main_Handle),
         K.Widget_States, K.Part_States,
   ...
   --  are reported: only the first failure is logged, so "SAVE" alone
   --  would not say where it happened.
   type Clip_Op is (Save, Narrow, Restore);
   type Clip_Site is
     (Gradient_Rect, Text_Item, Image_Item, Item_List, Widget_Tree);

   procedure Save_Clip
     (Renderer    : SDL_Renderer_Ptr;
      Prev        : access Adi.SDL.SDL_Rect;
      Was_Enabled : out Boolean;
      Saved       : out Boolean;
      Site        : Clip_Site);
   ...
   --  Narrow the clip to Rect. Returns whether SDL accepted it, so the
   --  caller's idea of the renderer state stays truthful; the caller
   --  decides whether to draw anyway.
   function Set_Clip
     (Renderer : SDL_Renderer_Ptr;
   ...
   procedure Report_Clip_Failure (Op : Clip_Op; Site : Clip_Site);
   ...
   is
      Ok : constant Boolean :=
        Boolean (SDL_SetRenderClipRect (Renderer, Rect));
   begin
      if not Ok then
         Report_Clip_Failure (Narrow, Site);
      end if;
      return Ok;
   end Set_Clip;
   ...
   procedure Set_Clip
     (Renderer : SDL_Renderer_Ptr;
      ...
      Site     : Clip_Site)
   is
      Unused : constant Boolean := Set_Clip (Renderer, Rect, Site);
   begin
      null;
   end Set_Clip;
   ...
   begin
      ...
      if not Boolean (Ok) then
         Report_Clip_Failure (Restore, Site);
      end if;
   end Restore_Clip;

   Clip_Failure_Reported : Boolean := False;

   procedure Report_Clip_Failure (Op : Clip_Op; Site : Clip_Site) is
   begin
   ...
   is
      Part_Handle : constant Widget_Style := Effective_Part_Handle (W, P);
      Main_Handle : constant Widget_Style :=
   ...
   begin
      ...
      for P in Part_Kind loop
         if W.Part_Styles (P).Enabled then
            declare
               H : constant Widget_Style := Effective_Part_Handle (W, P);
            begin
               if H /= Empty_Widget_Style
                 and then Uses_Widget_State (H, Changed)
               then
   ...
   begin
      ...
      for P in Part_Kind loop
         if P = Changed and then W.Part_Styles (P).Enabled then
            ...
            begin
               if H /= Empty_Widget_Style
                 and then Uses_Part_State (H, Changed_State)
               then
                  Inc_Sat (Perf_Style_Resolves);
                  Inc_Sat (Perf_Style_Resolves);
                  declare
   ...
   is
      ...
      Main_Uses  : constant Boolean :=
        Uses_Properties (Effective_Part_Handle (W, Main_Part));
      Worst      : Style_Diff_Kind := Diff_None;
   begin
      for P in Part_Kind loop
         if W.Part_Styles (P).Enabled then
  ...
  begin
   ...
   function Build_Content_Clip_Rect
     ...
     Content  : Rectangle;
      Clip_X   : Boolean;
      Clip_Y   : Boolean;
      Prev_Saved : Boolean;   --  Prev_Clip holds a saved rectangle
   ...
   begin
      ...
      if Clip_Replaced then
         ...
         --  Draw regardless: Set_Clip reports the failure, and a renderer
         --  that rejects a clip fails the draw calls too.
      ...
      if Clip_Replaced then
         Restore_Clip (Renderer, Prev_Clip'Access, Clip_Saved, Gradient_Rect);
      end if;
   ...
   begin
      ...
      if Use_Clip then
         Restore_Clip (Renderer, Prev_Clip'Access, Clip_Saved, Widget_Tree);
      end if;
   ...
   begin
      ...
      declare
         ...
         Scrollable_Y : constant Boolean :=
           Is_Scroll_Enabled (W);
         --  A percentage resolves against a containing block this query
         --  knows nothing about, so it is left to the layout pass that
         --  does and the axis counts as auto here. Resolving it against
         --  the widget's own geometry would reapply it every pass: a
         --  height: 50% child already at 200 would come back as 100,
         --  then 50.
         Sized_W : constant Boolean :=
           Style.Width.Kind = Fixed and then Style.Width.Size.Unit /= Pct;
         Sized_H : constant Boolean :=
           Style.Height.Kind = Fixed and then Style.Height.Size.Unit /= Pct;
