...
procedure Slider_Test is
   ...
   procedure Test_Create is
      S : constant Float_Slider.Slider_Widget_Access :=
        Float_Slider.Create (Min => 0.0, Max => 100.0, Value => 50.0);
   begin
      Put_Line ("Test: Float slider Create");
      Assert (Float_Slider.Get_Value (S.all) = 50.0,
              "Value should be 50.0");
      Assert (Float_Slider.Get_Min (S.all) = 0.0,
              "Min should be 0.0");
      Assert (Float_Slider.Get_Max (S.all) = 100.0,
              "Max should be 100.0");
   end Test_Create;
   ...
   begin
      ...
      Float_Slider.Set_Value (S.all, 75.0);
      Assert (Float_Slider.Get_Value (S.all) = 75.0,
   ...
   begin
      ...
      Float_Slider.Set_Value (S.all, 200.0);
      Assert (Float_Slider.Get_Value (S.all) = 100.0,
              "Value should be clamped to 100.0");
   end Test_Clamp_Above_Max;

   procedure Test_Clamp_Below_Min is
      S : constant Float_Slider.Slider_Widget_Access :=
        Float_Slider.Create (Min => 10.0, Max => 100.0, Value => 50.0);
   begin
      Put_Line ("Test: Float slider clamp below min");
      Float_Slider.Set_Value (S.all, 5.0);
      Assert (Float_Slider.Get_Value (S.all) = 10.0,
              "Value should be clamped to 10.0");
   end Test_Clamp_Below_Min;

   procedure Test_Create_Clamps is
      S : constant Float_Slider.Slider_Widget_Access :=
        Float_Slider.Create (Min => 0.0, Max => 50.0, Value => 999.0);
   begin
   ...
   procedure Test_Set_Range is
      S : constant Float_Slider.Slider_Widget_Access :=
        Float_Slider.Create (Min => 0.0, Max => 100.0, Value => 80.0);
   begin
      Put_Line ("Test: Float slider Set_Range reclamps value");
      Float_Slider.Set_Range (S.all, Min => 0.0, Max => 50.0);
      Assert (Float_Slider.Get_Value (S.all) = 50.0,
              "Value should be reclamped to new max");
      Assert (Float_Slider.Get_Max (S.all) = 50.0,
   ...
   begin
      ...
      Assert (Float_Slider.Get_Step (S.all) = 0.0,
              "Default step should be 0.0 (continuous)");
      Float_Slider.Set_Step (S.all, 10.0);
      Assert (Float_Slider.Get_Step (S.all) = 10.0,
   ...
   begin
      ...
      Assert (Float_Slider.Get_Orientation (S.all) = Float_Slider.Horizontal,
              "Default orientation should be Horizontal");
      Float_Slider.Set_Orientation (S.all, Float_Slider.Vertical);
      Assert (Float_Slider.Get_Orientation (S.all) = Float_Slider.Vertical,
   ...
   begin
      ...
      Assert (Has_Flag (S.all, Clickable),
              "Slider should be Clickable");
      Assert (Has_Flag (S.all, Focusable),
              "Slider should be Focusable");
      Assert (Has_Flag (S.all, Visible),
   ...
   procedure Test_Int_Create is
      S : constant Int_Slider.Slider_Widget_Access :=
        Int_Slider.Create (Min => 0, Max => 255, Value => 128);
   begin
      Put_Line ("Test: Integer slider Create");
      Assert (Int_Slider.Get_Value (S.all) = 128,
              "Value should be 128");
      Assert (Int_Slider.Get_Min (S.all) = 0,
              "Min should be 0");
      Assert (Int_Slider.Get_Max (S.all) = 255,
              "Max should be 255");
   end Test_Int_Create;

   procedure Test_Int_Clamp is
      S : constant Int_Slider.Slider_Widget_Access :=
        Int_Slider.Create (Min => 0, Max => 100, Value => 200);
   begin
      Put_Line ("Test: Integer slider clamp");
      Assert (Int_Slider.Get_Value (S.all) = 100,
   ...
   begin
      ...
      Int_Slider.Set_Step (S.all, 5);
      Assert (Int_Slider.Get_Step (S.all) = 5,
              "Step should be 5");
   end Test_Int_Step;
   ...
   begin
      ...
      Set_Geometry (S.all, (X => 0.0, Y => 0.0, Width => 200.0, Height => 24.0));
      S.Build_Items;
      S.On_Key_Down (SDL_SCANCODE_HOME, 0, False);
      Assert (Float_Slider.Get_Value (S.all) = 0.0,
              "Home should set value to min");
      S.On_Key_Down (SDL_SCANCODE_END, 0, False);
      Assert (Float_Slider.Get_Value (S.all) = 100.0,
