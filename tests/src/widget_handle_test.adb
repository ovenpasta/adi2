...
procedure Widget_Handle_Test is
   ...
   begin
      ...
      Assert (Is_Valid (H), "handle from Create should be valid");
      ...
      begin
         Assert (Is_Valid (H2), "label handle should be valid");
      end;
   ...
   begin
      ...
      Assert (not Is_Valid (Null_Handle), "Null_Handle should be invalid");
   ...
   begin
      ...
      Assert (Is_Valid (H), "before destroy: valid");
      Destroy (H);
      Assert (not Is_Valid (H), "after destroy: stale");
   ...
   begin
      ...
      Assert (not Is_Valid (H_Root), "root destroyed");
      Assert (not Is_Valid (H_C1), "child1 destroyed recursively");
      Assert (not Is_Valid (H_C2), "child2 destroyed recursively");
   ...
   begin
      ...
      Destroy (H);
      Pump_Widget_Store;
      Assert (not Is_Valid (H), "after destroy+pump: stale");
   end Test_Pump;
   ...
   begin
      ...
      Assert (Is_Valid (H), "handle valid");

      --  Get handle again from the same widget should match
      declare
         H2 : constant Widget_Handle := Get_Handle (Lbl.all);
      begin
         Assert (Is_Valid (H2), "second Get_Handle also valid");
      end;
   ...
   begin
      ...
      Assert (Ptr /= null, "Resolve_Handle returns non-null for valid handle");
      Assert (Ptr = Widget_Access (Btn),
              "Resolve_Handle returns same pointer");

      --  Resolve null handle should return null
      Assert (Resolve_Handle (Null_Handle) = null,
   ...
   begin
      ...
      Assert (Is_Valid (H), "Create_Handle returns valid handle");

      --  Resolve should give non-null
      Assert (Resolve_Handle (H) /= null,
   ...
   begin
      ...
      Assert (Label.Is_Valid (H), "Label_Handle should be valid");
      Assert (Is_Valid (W), "To_Widget_Handle should be valid");

      --  Get/Set text via handle
      Assert (Label.Get_Text (H) = "Typed",
              "Get_Text via handle should return initial text");
      Label.Set_Text (H, "Changed");
      Assert (Label.Get_Text (H) = "Changed",
      ...
      begin
         Assert (Label.Is_Valid (H2),
                 "Try_As_Label on label handle should succeed");
         Assert (Label.Get_Text (H2) = "Changed",
                 "Try_As_Label roundtrip preserves identity");
      end;

      --  Null handle
      Assert (not Label.Is_Valid (Label.Null_Label_Handle),
   ...
   begin
      ...
      Assert (Button.Is_Valid (H), "Button_Handle should be valid");
      Assert (Is_Valid (W), "To_Widget_Handle should be valid");
      ...
      begin
         Assert (True, "Connect_Clicked via handle compiled and ran");
      end;
   ...
   begin
      ...
      Assert (Box.Is_Valid (H), "Box_Handle should be valid");
      Assert (Is_Valid (WH), "To_Widget_Handle should be valid");
   ...
   begin
      ...
      Assert (Float_Slider.Is_Valid (H), "Slider_Handle should be valid");
      Assert (Is_Valid (WH), "To_Widget_Handle should be valid");

      --  Get/Set value via handle
      Assert (Float_Slider.Get_Value (H) = 50.0,
              "Get_Value via handle should return initial value");
      Float_Slider.Set_Value (H, 75.0);
      Assert (Float_Slider.Get_Value (H) = 75.0,
      ...
      declare
         ...
         pragma Unreferenced (Id);
      begin
         Assert (True, "Connect_Changed via handle compiled and ran");
      end;
      ...
      begin
         Assert (Float_Slider.Is_Valid (H2),
                 "Try_As_Slider on slider handle should succeed");
         Assert (Float_Slider.Get_Value (H2) = 75.0,
      ...
      begin
         Assert (not Float_Slider.Is_Valid (SH),
                 "Try_As_Slider on Label should return null handle");
      end;

      --  Null handle
      Assert (not Float_Slider.Is_Valid (Float_Slider.Null_Slider_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (+BxH), """+""(Box_Handle) returns valid Widget_Handle");
      Assert (Is_Valid (+LbH),
   ...
   begin
      ...
      begin
         ...
         Assert (True, "Disconnect_Changed via handle");
      end;
   ...
   begin
      ...
      begin
         ...
         Assert (True, "Connect/Disconnect_Toggled via handle");
      end;
      ...
      begin
         ...
         Assert (True, "Connect/Disconnect_Clicked via handle");
      end;
   ...
   begin
      ...
      begin
         ...
         Assert (True, "Connect/Disconnect_Value_Changed via handle");
      end;
      ...
      begin
         Assert (Float_Input.Is_Valid (H2),
                 "Try_As_Value_Input roundtrip should succeed");
      end;

      --  Null handle
      Assert (not Float_Input.Is_Valid (Float_Input.Null_Value_Input_Handle),
   ...
   begin
      ...
      begin
         ...
         Assert (True, "Int Connect/Disconnect_Value_Changed via handle");
      end;
   ...
   begin
      ...
      Assert (Is_Valid (H), "Image_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "Image To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Image_Handle := Try_As_Image (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Image roundtrip succeeds");
      end;

      --  "+" operator
      Assert (Adi.Widget.Is_Valid (+H),
      ...
      begin
         Assert (Get_Image (H) = null, "Image initially null");
      end;

      --  Null handle
      Assert (not Is_Valid (Null_Image_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Animated_Image_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "AI To_Widget_Handle valid");
      ...
      Assert (not Is_Playing (H), "AI not playing initially");
      Assert (not Is_Looping (H), "AI not looping without animation");

      --  Get_Animation should be null initially
      declare
         use type Adi.Animated_Image.Animated_Image_Access;
      begin
         Assert (Get_Animation (H) = null, "AI animation initially null");
      end;

      --  Null handle
      Assert (not Is_Valid (Null_Animated_Image_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "RLottie_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "RL To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant RLottie_Handle := Try_As_RLottie (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_RLottie roundtrip succeeds");
      end;
      ...
      Assert (not Is_Playing (H), "RL not playing initially");

      --  Is_Looping default (Desired_Looping defaults True)
      Assert (Is_Looping (H), "RL looping by default");

      --  Set_Looping / Is_Looping
      Set_Looping (H, False);
      Assert (not Is_Looping (H), "RL not looping after Set_Looping(False)");

      --  Get_Animation should be null initially
      declare
         use type Adi.RLottie.RLottie_Animation_Access;
      begin
         Assert (Get_Animation (H) = null, "RL animation initially null");
      end;

      --  Null handle
      Assert (not Is_Valid (Null_RLottie_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Html_View_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "HV To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Html_View_Handle := Try_As_Html_View (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Html_View roundtrip succeeds");
      end;
      ...
      --  Set_HTML / Get_HTML
      Assert (Get_HTML (H) = "", "HV initially empty HTML");
      Set_HTML (H, "<p>Hello</p>");
      Assert (Get_HTML (H) = "<p>Hello</p>", "HV Set/Get_HTML");

      --  Clear
      Clear (H);
      Assert (Get_HTML (H) = "", "HV empty after Clear");

      --  Content scale
      declare
         use type Adi.Core.Pixel_Type;
      begin
         Assert (Get_Content_Scale (H) = 1.0,
                 "HV default content scale 1.0");
         Set_Content_Scale (H, 2.0);
         Assert (Get_Content_Scale (H) = 2.0, "HV Set/Get_Content_Scale");
      end;

      --  Default stylesheet
      Assert (Get_Default_Stylesheet (H) = "",
              "HV default stylesheet initially empty");
      Set_Default_Stylesheet_String (H, "body { color: red; }");
      Assert (Get_Default_Stylesheet (H) = "body { color: red; }",
              "HV Set/Get default stylesheet string");

      --  Null handle
      Assert (not Is_Valid (Null_Html_View_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Animated_Widget_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "AW To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Animated_Widget_Handle := Try_As_Animated_Widget (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Animated_Widget roundtrip succeeds");
      end;

      --  Is_Playing should be false initially
      Assert (not Is_Playing (H), "Not playing initially");

      --  Null handle
      Assert (not Is_Valid (Null_Animated_Widget_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Switch_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "Switch To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Switch_Handle := Try_As_Switch (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Switch roundtrip succeeds");
      end;

      --  Set_Checked / Is_Checked
      Assert (not Is_Checked (H), "Switch not checked initially");
      Set_Checked (H, True);
      Assert (Is_Checked (H), "Switch checked after Set_Checked(True)");

      --  Is_Toggleable
      Assert (Is_Toggleable (H), "Switch should be toggleable");

      --  Null handle
      Assert (not Is_Valid (Null_Switch_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Text_Input_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "TI To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Text_Input_Handle := Try_As_Text_Input (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Text_Input roundtrip succeeds");
      end;

      --  Set_Text / Get_Text
      Assert (Get_Text (H) = "", "Text_Input initially empty");
      Set_Text (H, "Hello");
      Assert (Get_Text (H) = "Hello", "Text_Input Set/Get_Text");

      --  Null handle
      Assert (not Is_Valid (Null_Text_Input_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Text_Editor_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "TE To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Text_Editor_Handle := Try_As_Text_Editor (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Text_Editor roundtrip succeeds");
      end;

      --  Set_Text / Get_Text
      Set_Text (H, "Line1");
      Assert (Get_Text (H) = "Line1", "Text_Editor Set/Get_Text");

      --  Append_Text
      Append_Text (H, " more");
      Assert (Get_Text (H) = "Line1 more", "Text_Editor Append_Text");

      --  Set_Read_Only / Is_Read_Only
      Assert (not Is_Read_Only (H), "Not read-only initially");
      Set_Read_Only (H, True);
      Assert (Is_Read_Only (H), "Read-only after Set_Read_Only(True)");

      --  Null handle
      Assert (not Is_Valid (Null_Text_Editor_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Combo_Box_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "CB To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Combo_Box_Handle := Try_As_Combo_Box (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Combo_Box roundtrip succeeds");
      end;

      --  Add_Item / Option_Count / Get_Selected
      Assert (Option_Count (H) = 0, "Combo_Box initially empty");
      Add_Item (H, "Apple");
      Add_Item (H, "Banana");
      Assert (Option_Count (H) = 2, "Option_Count after 2 adds");
      Assert (Get_Selected_Index (H) = 1, "First item auto-selected");
      Assert (Get_Selected_Text (H) = "Apple", "Selected text is Apple");

      --  Set_Selected_Index
      Set_Selected_Index (H, 2);
      Assert (Get_Selected_Index (H) = 2, "Selected index after set");
      Assert (Get_Selected_Text (H) = "Banana", "Selected text is Banana");

      --  Clear_Items
      Clear_Items (H);
      Assert (Option_Count (H) = 0, "Option_Count after clear");

      --  Is_Open
      Assert (not Is_Open (H), "Combo_Box not open initially");

      --  Null handle
      Assert (not Is_Valid (Null_Combo_Box_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Dialog_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "Dlg To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Dialog_Handle := Try_As_Dialog (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Dialog roundtrip succeeds");
      end;
      ...
      begin
         Assert (Idx = 1, "Add_Button returns 1 for first button");
      end;
      Add_Button (H, "Cancel");
      ...
      Assert (not Is_Shown (H), "Dialog not shown initially");
      ...
      --  Null handle
      Assert (not Is_Valid (Null_Dialog_Handle),
   ...
   begin
      ...
      Assert (Is_Valid (H), "Stack_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "Stack To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant Stack_Handle := Try_As_Stack (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_Stack roundtrip succeeds");
      end;
      ...
      --  Get_Active
      Assert (Get_Active (H) = Page_A, "First page is active");
   ...
   begin
      ...
      Assert (Is_Valid (H), "List_Box_Handle should be valid");
      Assert (Adi.Widget.Is_Valid (WH), "LB To_Widget_Handle valid");

      --  Try_As roundtrip
      declare
         H2 : constant List_Box_Handle := Try_As_List_Box (WH);
      begin
         Assert (Is_Valid (H2), "Try_As_List_Box roundtrip succeeds");
      end;
      ...
      Assert (Get_Selection_Mode (H) = Single_Selection,
              "Selection mode is Single");
      Select_Row (H, 2);
      Assert (Is_Row_Selected (H, 2), "Row 2 selected");
      Assert (Get_Selected_Count (H) = 1, "Selected count is 1");

      --  Current row
      Set_Current_Row (H, 3);
      Assert (Get_Current_Row (H) = 3, "Current row is 3");

      --  Clear
      Clear_Selection (H);
      Assert (Get_Selected_Count (H) = 0, "Selected count 0 after clear");

      --  Clear_Rows
      Clear_Rows (H);
      Assert (Row_Count (H) = 0, "Row_Count 0 after Clear_Rows");

      --  Null handle
      Assert (not Is_Valid (Null_List_Box_Handle),
   ...
   begin
      ...
      Assert (Is_Visible (H), "Widget visible by default");
      Set_Visible (H, False);
      Assert (not Is_Visible (H), "Widget invisible after Set_Visible(False)");
      Set_Visible (H, True);
      Assert (Is_Visible (H), "Widget visible after Set_Visible(True)");

      --  Set_Disabled / Is_Disabled
      Assert (not Is_Disabled (H), "Widget not disabled by default");
      Set_Disabled (H, True);
      Assert (Is_Disabled (H), "Widget disabled after Set_Disabled(True)");
      Set_Disabled (H, False);
      Assert (not Is_Disabled (H), "Widget not disabled after Set_Disabled(False)");

      --  Set_Focusable (no getter via handle, just verify no crash)
      Set_Focusable (H, True);

      --  Set_Label / Get_Label
      Assert (Get_Label (H) = "", "Label empty by default");
      Set_Label (H, "my label");
      Assert (Get_Label (H) = "my label", "Get_Label after Set_Label");

      --  Mark_Dirty (just verify no crash)
      Mark_Dirty (H);

      --  Stale handle returns defaults
      Assert (not Is_Visible (Null_Handle), "Null_Handle Is_Visible=False");
      Assert (not Is_Disabled (Null_Handle), "Null_Handle Is_Disabled=False");
      Assert (Get_Label (Null_Handle) = "", "Null_Handle Get_Label=empty");
   ...
   begin
      ...
      begin
         Assert (R.Ptr /= null, "Borrow valid handle: Ptr not null");
      end;
      ...
      exception
         when Constraint_Error =>
            Assert (True, "Borrow Null_Handle raises Constraint_Error");
      end;
      ...
      declare
         R : constant Widget_Ref := Borrow (W);
      begin
         Assert (R.Ptr.all in Adi.Widget.Widget'Class,
   ...
   begin
      ...
      begin
         ...
         Assert (Result, "Bubble_Context_Menu reached Root's handler");
         Assert (Got_Menu, "Context menu callback was invoked");
      end;
