...
A Widget_Style is a four-byte handle and a Part_Style_Array twelve of
them, so the generator emits both as library-level constants: the sheet is
interned once at elaboration and every reader afterwards copies handles.
The Style_Rules each constant is built from stay expression functions,
which are called at elaboration and left behind.
"""
...
def generate_selector_registration_body(
    ...
    One procedure per selector, so a sheet registers in source order with
    one Static_Style_Entry temporary alive at a time.
    """
    ...
    for i, entry in enumerate(entries, start=1):
        lines += [
            ...
            #  Separate frames, one selector's entry in each.
...
def generate_ada_package(
    ...
    lines += [
        f"",
        f"--  The constants below intern as this package elaborates, so the",
        f"--  stores behind Intern_Rules and Build are wanted first.",
        f"pragma Elaborate_All (Adi.Widget_Styles);",
        f"",
    ...
    if root_properties:
        ...
        lines.append("   Root_Part_Styles : constant Part_Style_Array := Empty_Part_Styles;")
